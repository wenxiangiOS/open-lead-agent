...
class ProfileCollectionPolicy:
    ...
    def get_side_target(
        ...
        if self.can_actively_ask(profile, "monthly_income"):
            if main_target == "occupation" and self.can_enter_contact(profile):
    ...
    def can_enter_contact(self, profile: UserProfile) -> bool:
        ...
        has_background = has_occupation or has_education

        # 联系方式前至少要拿到稳定的地区和背景信息，避免在资料太薄时过早切到 contact。
        if not (has_location and has_background):
            return False

        # 优先要求年龄 + 婚况这两类强约束信息已知。
        if has_age and has_marital_status and core_quasi_collected >= 5:
            return True

        # 兜底：性别、年龄、地区、职业、学历、婚况已具备 5 项时也允许进入。
        return core_quasi_collected >= 5 and has_sex and has_location and has_occupation
