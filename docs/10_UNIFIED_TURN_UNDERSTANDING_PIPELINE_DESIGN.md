...
### 5.6 追问状态必须记录“实际问句”而不是“计划字段”（2026-04-22 补充）

为避免出现“决策计划问 A，但模型实际问了 B/C，下一轮用户回答 B/C 却被当成跑题”的问题，`last_question_state` 与短答槽位绑定必须遵守下面规则：

- `asked_fields/side_fields` 永远记录助手这轮**实际问到**的字段
- `planned_ask_field` 只作为决策痕迹保留，不能覆盖 `asked_fields`
- 如果模型问句和计划字段漂移：
  - `question_source` 记为 `actual_drifted`
  - `resume_profile_target` 保留原计划字段，供主线后续恢复
  - 下一轮统一理解、reply act 分类、字段放行都必须以实际问句字段为准

这条规则的目标不是鼓励模型漂移，而是即使发生漂移，也不能再丢掉用户已经按真实问句给出的答案。
...
5. 当 `asked_field=monthly_income` 且用户直接回答 `20k+ / 2万左右 / 年收入18+` 这类短答案时，应视为当前追问语境下的显式自述，允许走 `explicit_self_marker -> committed`，不能只挂 `provisional` 后直接切联系方式。
