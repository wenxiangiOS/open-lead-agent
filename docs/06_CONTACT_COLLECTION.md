...
- 对“微信和电话同号 / 同一个号”这类表达，如果统一理解层已经 committed `phone + wechat` 双通道，持久化层不能再用普通微信格式校验把数字型 `wechat` 误删；这类号码要按“双通道同号”保留。
...
- 用户主动给出正确电话或微信时，该联系方式允许直接被动入档
- 但被动入档本身不代表“联系方式主流程立即接管”
- 如果被动收下单个联系方式后，仍有未完成资料字段或未完成有效覆盖尝试，系统应先回到资料主线
- `monthly_income` 是特殊的后置字段：只有当它仍未完成覆盖且仍允许主动询问时，才应在已收下单个联系方式后优先恢复收入主线
- 如果 `monthly_income` 虽然为空，但已经有效询问过 1 次，或因为 FAQ / 疑虑 / 配合度 / 联系方式协商被 gate 掉，则不应再主动恢复收入主线
- 只有资料字段已完成时，才允许在已收下一个联系方式后继续主动追另一个联系方式

### 4.4.1 被动联系方式入档与主动联系方式推进的区别（新增）

- 用户主动发来合法电话/微信时，系统应直接收下并记档
- 该动作属于“被动联系方式入档”，不是“主动联系方式推进”
- 被动入档后不能默认立刻追另一个联系方式
- 被动入档后必须先检查：
  - 是否仍有未完成资料字段
  - 是否仍有未完成的有效覆盖尝试
- 只要资料主线还没完成，就先回到资料主线
- 其中 `monthly_income` 必须额外通过自己的主动询问 gate；如果 gate 不开，就不能因为字段为空而硬恢复到收入
- 只有在资料主线完成后，才允许基于已收下的一个联系方式继续主动追另一个联系方式
